...
import { getAccessToken, setAccessToken, deleteAccessToken } from 'lib/auth/tokens';
...
export function withApollo(PageComponent, { ssr = true } = {}) {
  ...
  if (ssr || PageComponent.getInitialProps) {
    WithApolloClient.getInitialProps = async context => {
      ...
      const { apolloClient, accessTokenManager } = initOnContext(context);
        ...
        if (ssr && AppTree) {
          try {
            ...
            const props = inAppContext
              ? { ...pageProps, apolloClient, accessTokenManager }
              : { pageProps: { ...pageProps, apolloClient, accessTokenManager } };
