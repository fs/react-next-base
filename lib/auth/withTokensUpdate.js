...
import { getAccessToken, setRefreshToken, deleteRefreshToken } from './tokens';
...
const updateTokensServerSide = async ({ req, res, apolloClient }) => {
  try {
    ...
    if (!res.writableEnded) setRefreshToken({ refreshToken, res });
  } catch (error) {
    console.error(error);
    if (!res.writableEnded) deleteRefreshToken({ res });
  }
};
...
const WithTokensUpdate = Page =>
  class WithTokensUpdate extends React.Component {
